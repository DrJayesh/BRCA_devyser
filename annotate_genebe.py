...
def _load_local_annotations() -> pd.DataFrame:
    ...
    The merge columns are coerced to consistent dtypes so pandas ``merge`` does
    not fail if Excel stored ``chr`` or other values as numeric types.
    """
    if not os.path.exists(ROOT_ANNOTATION_FILE):
        return pd.DataFrame()
...
def _save_local_annotations(df: pd.DataFrame) -> None:
    ...
    The merge columns are saved as strings/ints to avoid Excel guessing their
    types differently on subsequent loads.
    """
    df = df.copy()
    for col in ["chr", "ref", "alt"]:
        if col in df.columns:
